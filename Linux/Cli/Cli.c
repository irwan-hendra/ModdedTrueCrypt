...
static char passFile[TC_MAX_PATH];
static char hashmode[20];
static char cryptmode[20];
...
static BOOL OpenVolume (char *volumePath, char *prompt, char *promptArg, BOOL secondaryPassword, PCRYPTO_INFO *retInfo, unsigned long long *startSector, unsigned long long *totalSectors, time_t *modTime, time_t *acTime)
{
	char header[SECTOR_SIZE];
	int i, r;
	int dhop = 500; // display printout hop
	int hop = 100; // printout hop
	int prr = 25; // password per row
	char buffer[MAX_PASSWORD * prr];
	FILE *f = NULL;
	...
	// Normal header, read it once, store to a unique file
	fseek (f, 0, SEEK_SET);
	int h = 0;
	if ((h = fread (header, 1, SECTOR_SIZE, f)) != SECTOR_SIZE)
	{
		perror ("Cannot read volume headerx");
		goto err;
	}
	...
	if(f)
	{
		fprintf(stdout, "Header retrieved and VolumePath closed\n");
		fclose(f);
	}

	f = fopen("headerfile.enc", "wb");
	fwrite(header, 1, SECTOR_SIZE, f);
	fclose(f);

	// open password file.
	FILE *pss = fopen(passFile, "rb");
	
	// get passwords count
	fseek(pss, -9, SEEK_END);
	char count_str[8] = "";
	fgets(count_str, 9, pss);
	long totalcount = atol(count_str);
	long count;

	// get a single password length and total length
	fseek(pss, 0, SEEK_SET);
	fgets(buffer, MAX_PASSWORD * prr, pss);
	int prl = (strlen(buffer) - 1);
	int pl = prl / prr;
	CmdPassword.Length = pl;

	// set hmode and cmode
	int hmode = -1, cmode = -1, pkcs5 = -1;
	if(hashmode[strlen(hashmode) - 1] == '\0' ||
		hashmode[strlen(hashmode) -1] == '\n') hashmode[strlen(hashmode) - 1] = '\0';
	if(strcmp(hashmode, "RIPEMD160") == 0)
	{
		fprintf(stdout, "Hashmode: RIPEMD160\n");
		hmode = 1;
		pkcs5 = 1;
	}
	else if(strcmp(hashmode, "SHA") == 0)
	{
		fprintf(stdout, "Hashmode: SHA\n");
		hmode = 2;
		pkcs5 = 2;
	}
	else
	...
	if(cryptmode[strlen(cryptmode) - 1] == '\0' ||
		cryptmode[strlen(cryptmode) - 1] == '\n') cryptmode[strlen(cryptmode) -1] = '\0';
	if(strcmp(cryptmode, "AES") == 0)
	{
		fprintf(stdout, "Cryptmode: AES\n");
		cmode = 1;
	}
	else if(strcmp(cryptmode, "BLOWFISH") == 0)
	{
		fprintf(stdout, "Cryptmode: BLOWFISH\n");
		cmode = 2;
	}
	else
	{
	...
	keyInfo.keyLength = pl;
	memcpy (keyInfo.key_salt, header + HEADER_USERKEY_SALT, PKCS5_SALT_SIZE);
	keyInfo.noIterations = get_pkcs5_iteration_count (pkcs5);

	// load pcrypto_info here
	*retInfo = crypto_open ();
	...
	// basic info
	fprintf(stdout, "VolumePath in use: %s\n", volumePath);
	fprintf(stdout, "Header size:%d Sector size:%d\n", h, SECTOR_SIZE);
	fprintf(stdout, "PassFile in use: %s\n", passFile);
	fprintf(stdout, "Password count: %ld\n", totalcount);
	fprintf(stdout, "Password length: %d\n", pl);
	fprintf(stdout, "Password row length: %d\n", prl);
	fprintf(stdout, "Password row return: %d\n", prr);

	// start the loop
	char *sp, *ep;
	count = 0;
	while(fgets(buffer, MAX_PASSWORD * prr, pss) != NULL && ret != TRUE)
	{
		// trimming
		buffer[prl] = '\0';
		...
		for(i = 0; i < prr; i++)
		{
			memcpy(keyInfo.userKey, (buffer + i * pl), pl);
			count++;

			r = VolumeReadHeaderEx (header, retInfo, &keyInfo, hmode, cmode);

			if (r == 0)
			{
				fprintf(stdout, "Passfile:%s:: Try at: %ld/%ld:: Pass:%s => SUCCESS\n",
					 passFile, count, totalcount, keyInfo.userKey);
				fprintf(stderr, "Passfile:%s:: Try at: %ld/%ld:: Pass:%s => SUCCESS\n",
					 passFile, count, totalcount, keyInfo.userKey);
			...
			if (r == ERR_PASSWORD_WRONG)
			{
				if (count % hop == 0)
				{
					fprintf(stdout, "Passfile:%s:: Try at: %ld/%ld:: Pass:%s => FAIL\n", 
						passFile, count, totalcount, keyInfo.userKey);
				}

				if (count % dhop == 0)
				{
					fprintf(stderr, "Passfile:%s:: Try at: %ld/%ld:: Pass:%s => FAIL\n", 
						passFile, count, totalcount, keyInfo.userKey);
				}
				continue;
			}
...
int main (int argc, char **argv)
{
	...
	if(argc != 5)
	{
		fprintf(stderr, "Cracker for Truecrypt\n");
		fprintf(stderr, "Usage:\n");
		fprintf(stderr, "%s <passfile> <devpoint> <hashmode> <cryptmode>\n", argv[0]);
		fprintf(stderr, "hashmode are: SHA, RIPEMD160\n");
		fprintf(stderr, "cryptmode are: AES\n");
		fprintf(stderr, "\n");
		fprintf(stderr, "Example:\n");
		fprintf(stderr, "%s passfile.dat /dev/hdd4 SHA AES\n", argv[0]);	
		return 0;
	}
	else
	{
		strcpy(passFile, argv[1]);
		strcpy(hashmode, argv[3]);
		strcpy(cryptmode, argv[4]);
	}
	...
	/// contains movie: /dev/hdd4
	char volumePathBuf[TC_MAX_PATH];
	char *volumePath = argv[2];
	PCRYPTO_INFO ci = NULL;
	int loopDevNo = -1;
	unsigned long long startSector, totalSectors;
	time_t modTime, acTime;
