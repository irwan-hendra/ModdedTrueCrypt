...
VolumeReadHeader (char *encryptedHeader, Password *password, PCRYPTO_INFO *retInfo)
{
	...
	// Test only AES
	cryptoInfo->ea = 1;
	int blockSize = CipherGetBlockSize (EAGetFirstCipher (1));
	status = EAInit (1, dk + DISK_IV_SIZE, cryptoInfo->ks);
	...
	DecryptBuffer ((unsigned __int32 *) (header + HEADER_ENCRYPTEDDATA), HEADER_ENCRYPTEDDATASIZE, cryptoInfo);
...
int
VolumeReadHeaderEx (char *encryptedHeader, PCRYPTO_INFO *retInfo, KEY_INFO *keyInfo, int hmode, int cmode)
{
	char header[HEADER_SIZE];
	unsigned char *input = (unsigned char *) header;
	int nKeyLen;
	...
	switch(hmode)
	{
		...
		case 1:
		{
			derive_key_ripemd160Ex(keyInfo->userKey, keyInfo->keyLength, keyInfo->key_salt, PKCS5_SALT_SIZE, keyInfo->noIterations, dk, DISK_IV_SIZE + EAGetLargestKey());
			break;
		}
		...
		case 2:
		{
			derive_key_sha1Ex(keyInfo->userKey, keyInfo->keyLength, keyInfo->key_salt,
				PKCS5_SALT_SIZE, keyInfo->noIterations, dk, DISK_IV_SIZE + EAGetLargestKey());
			break;
		}
	...
	status = EAInit (cmode, dk + DISK_IV_SIZE, (*retInfo)->ks);
	if (status == ERR_CIPHER_INIT_FAILURE) return ERR_PASSWORD_WRONG;

	// Copy header for decryption and init an encryption algorithm
	memcpy (header, encryptedHeader, SECTOR_SIZE);
	memcpy ((*retInfo)->iv, dk, DISK_IV_SIZE);

	if (!EAInitMode ((*retInfo)))
	{
		status = ERR_MODE_INIT_FAILED;
		goto err;
	}
	...
	// Try to decrypt header 
	DecryptBuffer ((unsigned __int32 *) (header + HEADER_ENCRYPTEDDATA), HEADER_ENCRYPTEDDATASIZE, (*retInfo));
	input = header + HEADER_ENCRYPTEDDATA;

	// Magic 'TRUE'
	if (mgetLong (input) != 0x54525545) return ERR_PASSWORD_WRONG;


////////////////////////////////end here unless the password is correct
	...
	// Volume creation time
	(*retInfo)->volume_creation_time = mgetInt64 (input);

	// Header creation time
	(*retInfo)->header_creation_time = mgetInt64 (input);

	// Hidden volume size (if any)
	(*retInfo)->hiddenVolumeSize = mgetInt64 (input);

	// Disk key
	nKeyLen = DISKKEY_SIZE;
	memcpy (keyInfo->key, header + HEADER_DISKKEY, nKeyLen);

	memcpy ((*retInfo)->master_key, keyInfo->key, nKeyLen);
	memcpy ((*retInfo)->key_salt, keyInfo->key_salt, PKCS5_SALT_SIZE);
	(*retInfo)->pkcs5 = pkcs5;
	(*retInfo)->noIterations = keyInfo->noIterations;

	// Init the encryption algorithm with the decrypted master key
	status = EAInit ((*retInfo)->ea, keyInfo->key + DISK_IV_SIZE, (*retInfo)->ks);
	if (status == ERR_CIPHER_INIT_FAILURE)
		goto err;

	// The secondary key (LRW mode) for the data area
	memcpy ((*retInfo)->iv, keyInfo->key, DISK_IV_SIZE);
