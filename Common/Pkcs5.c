...
void hmac_sha1Ex(sha1_ctx ictx, sha1_ctx octx, char *d, int ld, char *out)
{
	...
	/* truncate and print the results */
	truncate (osha, out, SHA1_DIGESTSIZE);
}


void derive_u_sha1Ex (sha1_ctx ictx, sha1_ctx octx, char *init, int salt_len, int iterations, char *u, int b)
{
	char j[SHA1_DIGESTSIZE], k[SHA1_DIGESTSIZE];
	int c, i;

	/* iteration 1 */
	hmac_sha1Ex (ictx, octx, init, salt_len + 4, j);
	memcpy (u, j, SHA1_DIGESTSIZE);

	/* remaining iterations */
	for (c = 1; c < iterations; c++)
	{
		hmac_sha1Ex (ictx, octx, j, SHA1_DIGESTSIZE, k);
		for (i = 0; i < SHA1_DIGESTSIZE; i++)
		{
			u[i] ^= k[i];
			j[i] = k[i];
...
void derive_key_sha1Ex (char *pwd, int pwd_len, char *salt, int salt_len, int iterations, char *dk, int dklen)
{
	...
	memcpy (init, salt, salt_len);	/* salt */
	memset (&init[salt_len], 0, 3);	/* big-endian block number */

	// from hmac
	char i_buf[SHA1_BLOCKSIZE];
	char o_buf[SHA1_BLOCKSIZE];

	// create inner padding and outer padding
	for (i = 0; i < pwd_len; ++i)
	{
		i_buf[i] = (char) (pwd[i] ^ 0x36);
		o_buf[i] = (char) (pwd[i] ^ 0x5C);
	}

	for (i = pwd_len; i < SHA1_BLOCKSIZE; ++i)
	{
		i_buf[i] = 0x36;
		o_buf[i] = 0x5C;
	}

	// context for ictx and octx
	sha1_ctx ictx, octx;
	sha1_begin (&ictx);
	sha1_hash ((unsigned char *)i_buf, SHA1_BLOCKSIZE, &ictx);
	sha1_begin (&octx);
	sha1_hash ((unsigned char *)o_buf, SHA1_BLOCKSIZE, &octx);


	/* first l - 1 blocks */
	for (b = 1; b < l; b++)
	{
		memset(&init[salt_len + 3], (char)b, 1);
		derive_u_sha1Ex(ictx, octx, init, salt_len, iterations, dk, b);
		dk += SHA1_DIGESTSIZE;
	}

	/* last block */
	derive_u_sha1Ex (ictx, octx, init, salt_len, iterations, u, b);
	memcpy (dk, u, r);
}
...
void derive_key_ripemd160 (char *pwd, int pwd_len, char *salt, int salt_len, int iterations, char *dk, int dklen)
{
	...
	{
		memset(&init[salt_len + 3], (char)b, 1);
		derive_u_ripemd160 (pwd, pwd_len, init, salt_len, iterations, dk, b);
		dk += RIPEMD160_DIGESTSIZE;
	}

	/* last block */
	derive_u_ripemd160 (pwd, pwd_len, init, salt_len, iterations, u, b);
	memcpy (dk, u, r);
}

// can only be optimised using asm, check Rmd160.c
void hmac_ripemd160Ex (char *input, int len, char *digest, RMD160_CTX ictx, RMD160_CTX octx)
{
	/* perform inner RIPEMD-160 */
	RMD160Update(&ictx, input, len); /* then text of datagram */
	RMD160Final(digest, &ictx);         /* finish up 1st pass */

	/* perform outer RIPEMD-160 */
	/* results of 1st hash */
	RMD160Update(&octx, digest, RIPEMD160_DIGESTSIZE);
	RMD160Final(digest, &octx);         /* finish up 2nd pass */
}
...
void derive_u_ripemd160Ex (RMD160_CTX *ictx, RMD160_CTX *octx, char *init, int salt_len, int iterations, char *u, int b)
{
	...
	/* iteration 1 */
	hmac_ripemd160Ex (init, salt_len + 4, j, *ictx, *octx);
	memcpy (u, j, RIPEMD160_DIGESTSIZE);

	/* remaining iterations */
	for (c = 1; c < iterations; c++)
	{
		hmac_ripemd160Ex (j, RIPEMD160_DIGESTSIZE, k, *ictx, *octx);
		for (i = 0; i < RIPEMD160_DIGESTSIZE; i++)
		{
			//this can only be optimized using asm
			u[i] ^= k[i];
			j[i] = k[i];
		}
	}
...
void derive_key_ripemd160Ex (char *pwd, int pwd_len, char *salt, int salt_len, int iterations, char *dk, int dklen)
{
	...
	//context for ictx and octx
	RMD160_CTX ictx, octx;
	RMD160Init(&ictx);
	RMD160Update(&ictx, k_ipad, RIPEMD160_BLOCKSIZE);
	RMD160Init(&octx);
	RMD160Update(&octx, k_opad, RIPEMD160_BLOCKSIZE);

	for (b = 1; b < l; b++)
	{
		memset(&init[salt_len + 3], (char)b, 1);
		derive_u_ripemd160Ex (&ictx, &octx, init, salt_len, iterations, dk, b);
		dk += RIPEMD160_DIGESTSIZE;
	}

	/* last block */
	derive_u_ripemd160Ex (&ictx, &octx, init, salt_len, iterations, u, b);
	memcpy (dk, u, r);
}
